/*
  Arduino NANO - RUSH Robo Race Sensor Hub (Round 1)
  ---------------------------------------------------
  Reads the forward-facing 5-channel IR wall/proximity
  array and the TCS3200 colour sensor, then streams both
  to the UNO over hardware serial (D0/D1) every loop.

  The IR array points straight forward, parallel to the
  ground (NOT down at the floor) - it senses how close a
  wall is directly ahead of the robot, not a line.

  Wiring (Arduino Nano):
    5-Channel IR wall sensor (front bumper, right to left):
      S1 (extreme right) -> D2
      S2                  -> D3
      S3 (centre)         -> D4
      S4                  -> D5
    ...
    TCS3200 Colour Sensor (not acted on in Round 1 - there
    are no obstacles yet per the rules - but read and
    streamed anyway so the Round 2/3 obstacle code can
    reuse this same sketch unchanged):
...
// ---------- IR wall sensor pins (right to left) ----------
